...
export function PricingBlock() {
  ...
  return (
    <>
      <section id="pricing" className="relative py-12 md:py-12">
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        <motion.div
          ref={cardsRef}
          className={`${isMobile ? 'flex flex-col gap-6' : 'flex flex-col md:flex-row justify-center items-center md:items-end relative z-10 pt-14 gap-6 md:gap-8'}`}
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          {pricingCards.map((card, index) => (
            <PricingCard key={index} card={card} index={index} openModal={openModal} />
          ))}
        </motion.div>
      </div>
    </section>
...
function PricingCard({ card, index, openModal }: { card: PricingCard; index: number; openModal: () => void }) {
  ...
  return (
    <motion.div
      ref={ref}
      className={`relative transition-transform duration-300 ease-out hover:scale-105 ${!isMobile && card.isMain ? 'md:-mt-8' : ''}`}
      initial={{ opacity: 0, y: 20 }}
    ...
    >
              <div
          className="relative rounded-[18.77px] transition-all duration-300 ease-out hover:shadow-2xl w-full"
          style={{
            width: isMobile ? '100%' : '100%',
            maxWidth: isMobile ? '100%' : (card.isMain ? '380px' : '360px'),
        ...
        <div className="h-full flex flex-col relative z-10" style={{ 
          ...
          <div style={{ marginTop: 'auto', flexShrink: 0, minHeight: isMobile ? '52px' : '55.9px' }}>
            <button
              onClick={openModal}
              className="w-full py-4 px-6 font-medium transition-all duration-300 hover:scale-105 relative"
              style={{
                width: isMobile ? '100%' : '100%',
            ...
            >
              {/* Gradient border for side cards (desktop) */}
              {!card.isMain && !isMobile && (
                <div
              ...
              <span
                style={{
                  background: card.isMain ? 'none' : 'linear-gradient(279.19deg, #80E5FF -11.2%, #545C9E 41.14%)',
                  WebkitBackgroundClip: card.isMain ? 'none' : 'text',
                  WebkitTextFillColor: card.isMain ? 'inherit' : 'transparent',
                  backgroundClip: card.isMain ? 'none' : 'text',
                  position: 'relative',
                  zIndex: 1
                }}
              >
