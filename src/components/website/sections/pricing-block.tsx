  ...
  return (
    <section className="relative py-12 md:py-12" id="pricing" ref={sectionRef as any}>
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        {/* Header */}
        <motion.div
          ref={(el) => { headerRef(el); headerElRef.current = el as HTMLDivElement; }}
        ...
        <motion.div
          ...
          animate={isMobile ? { opacity: 1, y: 0 } : getConditionalAnimate(cardsInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 })}
...
function PricingCard({ card, index }: { card: PricingCard; index: number }) {
  ...
  return (
    <motion.div
      ...
      initial={isMobile ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
      animate={isMobile ? { opacity: 1, y: 0 } : getConditionalAnimate(inView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 })}
